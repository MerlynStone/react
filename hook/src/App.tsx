...
const InternalCalendar: React.ForwardRefRenderFunction<CalendarRef, CalendarProps> = (props, ref) => {
  ...
  const endtDayOfMonth = (year: number, month: number,day:number) => {
  ...
  const renderDays = () => {
    ...
    const daysPreCount = daysOfMonth(date.getFullYear(), date.getMonth()-1);
    const endDay = endtDayOfMonth(date.getFullYear(), date.getMonth(),daysCount);
    console.log(endDay)


    for (let i = 0; i < firstDay; i++) {
      days.push(<div key={`empty-${i}`} className="empty">{daysPreCount-firstDay+i+1}</div>);
    }
    ...
    for (let i = 0; i < (7-endDay-1); i++) {
      days.push(<div key={`empty-e${i}`} className="empty">{i+1}</div>);
    }
    return days;
  };
